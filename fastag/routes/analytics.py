...
@analytics_bp.route('/api/denied-nonfastag-activity-feed')
def denied_nonfastag_activity_feed():
    """Return a feed of unique denied non-FASTag events (last 24h), deduped by tag_id or vehicle_number, for activity feed display."""
    try:
        db = get_db()
        denied_rows = db.execute('''
            SELECT 
