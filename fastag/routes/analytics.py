...
@analytics_bp.route('/api/denied-fastag-unique')
def denied_fastag_unique():
    """Return unique denied FASTag tags (last 24h), deduped by tag_id or vehicle_number, with last denied event details."""
    try:
        db = get_db()
        # Get all denied FASTag events in last 24h
        denied_rows = db.execute('''
            ...
            ORDER BY al.timestamp DESC
        ''').fetchall()
        # Deduplicate by tag_id or vehicle_number (if either repeats, only show once)
        seen_tag_ids = set()
